      ...
      <Toolbar >
        <Container>
          <Box display="flex" justifyContent="space-between">
            {
              user?.rol_nombre === "Administrador" || user?.rol_nombre === "SuperAdministrador"
              ? (
                <Box sx={{ display: { xs: 'none', sm: 'flex', alignItems: "center" } }}>
              
                  {navItems.map((item, index) => (
                    <Button onClick={()=>navigateTo(item.url)} key={index} style={{ color: '#fff', background: "none", fontSize:"16px", fontWeight: "100", textDecoration: "none", marginRight: "10px", padding: "0"}}>
                      {item.name}
                    </Button>
                  ))}
            ...
            <Box display="flex" >  
...

              {
                isLoggedIn 
                ? (
                  <Button onClick={ onLogout } variant="text" style={{ color: "white", fontWeight: "100", fontSize: "16px" }}>
                  Cerrar sesión
                  </Button>
                )
                : (
                  <Button onClick={()=> navigateTo(`${BASE_URL}/auth`)} variant="text" style={{ color: "white", fontWeight: "100", fontSize: "16px" }}>
                  Iniciar sesión
                  </Button>
                )
              }
