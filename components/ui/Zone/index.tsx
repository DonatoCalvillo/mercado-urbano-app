...
export const Zone: FC<Props> = ({
  ...
  const [lugarRegistrado, setLugarRegistrado] = useState<string | null>(
    numeroLugar
  );
  ...
  useEffect(() => {
    if (inscrito === 1) setStatus("Inscrito");
    else if (inscrito === 0) setStatus("Inscripcion");
  }, []);
  ...
  const reservar = async () => {
    ...
    try {
      const { data } = await mercadoUrbanoApi.get(
        "/event/validateInscription",
        {
          headers: { Authorization: `bearer ${token}` },
        }
      );

      if (data.status === "OK") setStatus("Registro");
  ...
  const elegirLugar = async () => {
    const token = Cookies.get("token")?.toString();
    try {
      const { data } = await mercadoUrbanoApi.post<ILugarSeleccionado>(
        "/event/setLugar",
        {
            ...
            <FormControl fullWidth>
              ...
              >
                {lugares?.map((lugar, index) => {
                  return (
                    <MenuItem key={index} value={lugar.lugar_id}>
                      {lugar.area_nombre[0]}
                      {lugar.lugar_numero}
                    </MenuItem>
                  );
